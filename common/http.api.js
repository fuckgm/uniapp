...
const install = (Vue, vm) => {
	...
	const setting = {
		...
		//添加银行卡其他
		saveCardOther: (params) => vm.$u.post("/user/cash_save_other",params),
		//获取银行卡信息
		getCard: () => vm.$u.post("/user/cash_info"),
		//获取银行卡信息國際
		getCardInternational: () => vm.$u.post("/user/cash_info_international"),
		//获取其他银行卡信息
		getCardOther: () => vm.$u.post("/user/cash_info_other"),
		//发送短信
	...
	const wallet = {
		...
		getDetail: (currency,type,isreal) => vm.$u.post("/wallet/detail",{currency,type,isreal}),
		// 获取特定币种特定方式的帐变记录
		getLegalLog: (currency,type,page,isreal) => vm.$u.post("/wallet/legal_log",{currency,type,page,isreal}),
		// 获取币币划转的可用列表
	...
	const market = {
		...
		//获取已购买的秒合约列表
		getSecondsList: (match_id,status,page = 1,limit = 5,isreal) => vm.$u.get("/microtrade/lists",{match_id,status,page,limit,isreal}),	
		//获取秒合约的秒数
		getSecondsSeconds: () => vm.$u.get("/microtrade/seconds"),	
		//购买秒合约
		buySeconds: (match_id,currency_id,type,seconds,number,isreal) => vm.$u.post("/microtrade/submit",{match_id,currency_id,type,seconds,number,isreal}),
			 //获取秒合约的数量
			 getSecondsNums: () => vm.$u.get("/microtrade/nums"),
	...
	const lockming = {
		...
		// 获取锁仓挖矿的订单
		getLockmingOrder: (page,limit,status) => vm.$u.get("/lh/deposit/order",{page,limit,status}),	
		// 认购锁仓挖矿
