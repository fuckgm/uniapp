			...
			<view class="p-30 border-bottom text-white" v-for="item in list">
				<view class="d-flex">
					<view class="d-flex align-items-center">
						...
						<text class="ml-8">{{$t(item.currency_name)}}</text>
		...
		watch:{
			profitPrice(){
				...
				if(currentItem.type == 1){
					if(profitPrice > currentItem.price){
						expectedProfit = (profitPrice - currentItem.price) * currentItem.share;
					}else{
						expectedProfit = '0.00';
					}
					
				}else{
					if(currentItem.price > profitPrice){
						expectedProfit = (currentItem.price -profitPrice) * currentItem.share;
					}else{
						expectedProfit ='0.00';
					}
				...
				this.expectedProfit = Number(expectedProfit).toFixed(4)
			},
			lossPrice(){
				let expectedLoss = 0
				...
				this.expectedLoss = Number(expectedLoss).toFixed(4)
			}
		},
		onHide(){
