	...
	<view>
		...
		<view class="mx-30 pt-30">
			<view class="d-flex-between-center">
				...
				<u-subsection :list="nav" style="width: 630rpx;" v-if="showNav"  @change="(index)=>currentNav=index" inactive-color="#fff" :current="currentNav"></u-subsection>
			</view>
			...
			<view class="d-flex-between-center mt-30 text-white ">
				<view class="d-flex align-items-center font-weight-bold opacity-75" @click="showSelectCoin=true">
					...
					<text class="font-size-40 mx-10">{{$t(currency_name)}}</text>
				...
				<view class="d-flex align-items-center">
					<!-- <text :color="$baseColor" class="mr-20 isreal" @click="$utils.setReal(`/pages/transaction/index`)" v-if="currentNav == 1 && isreal==0" >{{$t("real")}}</text>
					<text :color="$baseColor" class="mr-20 isreal" @click="$utils.setReal(`/pages/transaction/index`)" v-if="currentNav == 1 && isreal==1">{{$t("demo")}}</text> -->
									<!-- 合约订单 -->
			...
			<view class="d-flex overflow-hidden">
				<view class="text-white w-50">
					...
					<view class="d-flex-between-center px-10 py-16 position-relative font-size-22" v-for="item in buyList">
						...
						<view class="position-absolute opacity-20" style="top: 0;bottom: 0;right: 0;" :style="{background:$upColor,width:(item[1]/buyListCount*100*10+'%')}"></view>
					</view>
				</view>
				<view class="text-white w-50">
					...
					<view class="d-flex-between-center px-10 py-16 position-relative font-size-22" v-for="item in sellList">
						...
						<text class="opacity-75" >{{item[1]}}</text>
						<view class="position-absolute opacity-20" style="top: 0;bottom: 0;left: 0;" :style="{background:$downColor,width:(item[1]/sellListCount*100*10+'%')}"></view>
					</view>
				</view>
			</view>
		</view>
			...
			<u-popup v-model="showBuyConfirm" mode="center" length="90%" contentBackgroundColor="rgba(0, 0, 0, .4)"
				...
				<view class="p-30">
					...
					<view class="d-flex-between-center border-bottom-white py-16">
						...
						<text class="font-weight-bold">{{$t(currency_name)}}</text>
					</view>
					<view class="d-flex-between-center border-bottom-white py-16">
					...
					<view class="d-flex-between-center border-bottom-white py-16">
						...
						<text class="font-weight-bold">{{profit}}</text>
					</view>
					<view class="border-bottom-white py-16">
						...
						<view class="d-flex-between-center mt-16" v-if="buyDirection == 1">
							<text v-for="(item,index) in timers" :key="index" class="tag w-19 text-center font-size-30 d-block" :class="currentTimer == item ? 'tag-success py-12' : 'tag-plain-success py-10'" @click="currentTimer=item,getrate=rates[index],calcrate()">{{item}}s {{rates[index]}}%</text>
							
						</view>
						<view class="d-flex-between-center mt-16" v-else-if="buyDirection == 2">
							<text v-for="(item,index) in timers" :key="index" class="tag w-19 text-center font-size-30 d-block" :class="currentTimer == item ? 'tag-error py-12' : 'tag-plain-error py-10'" @click="currentTimer=item,getrate=rates[index],calcrate()">{{item}}s {{rates[index]}}%</text>
						</view>
					</view>
					<view class="py-16">
						<text class="opacity-50">{{i18n.number}}</text>
						<view class="d-flex-between-center mt-16">
							<text v-for="item in nums" class="tag w-19 text-center font-size-30 d-block" :class="number == item ? 'tag-success py-12' : 'tag-plain-success py-10'" @click="number=item,calcrate()">{{item}} USDT</text>
						</view>
						<input type="number" v-model="number" class="trade-input mt-10 " @input="calcrate()">
...
<script>
	export default {
		...
		onLoad(options) {
			...
			this.from = from || ''
			this.currency_name = currency_name || "LTC"
			this.legal_name = legal_name || "USDT"
			this.currency_id = currency_id || 5
		...
		methods: {
			...
			getSecondsSeconds(){
				this.$u.api.market.getSecondsSeconds().then(res => {
					...
					this.rates = res.message.map(item => item.profit_ratio)
					this.currentTimer = this.timers[0]
					this.getrate = this.rates[0]
				})
			},
			//计算收益率
			calcrate(){
				this.profit=this.number*(this.getrate)/100
				
			},
			//获取秒合约的购买数量
			getSecondsNums(){
				this.$u.api.market.getSecondsNums().then(res => {
					this.nums = res.message.map(item => item.number)
					// console.log(this.nums)
					// console.log(res.message)
					this.number = this.nums[0]
				})
			},
			// 获取所有行情
			getQuotation() {
			...
			getPayable(){
				...
				this.$u.api.market.getPayable().then(res=>{
					const data = res.message.find(item=>item.id == 3) //显示USDT余额
					// const data = res.message.find(item=>item.id == this.currency_id)
					// console.log(data.user_wallet)
					this.microInsurance = data.user_wallet.micro_with_insurance
					if(this.isreal==1){
						this.microInsurance = data.user_wallet.moni_balance
					}
				})
			},
			//提交购买
			submit(){
				...
				this.$u.throttle(()=>{
					...
					this.$u.api.market.buySeconds(id,legal_id,buyDirection,currentTimer,number,this.isreal).then(res=>{
						...
						this.countDown = setInterval(()=>{
							...
							if(this.countDownTime >= 1){
								...
								this.$u.api.market.getResult(this.buyInfo.id).then((res) => {
									...
									this.endtime = _data.complete_at
									this.endprice = _data.end_price
									// console.log(res);
								});
								// this.continueTrade()
							}
