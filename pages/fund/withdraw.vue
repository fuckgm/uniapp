	...
	<view class="text-white">
		...
		<view class="mx-36 py-20">
			<view class="bg-black px-30 py-16 box-shadow border-radius-10">
				<view class="select-coin" @click="showSelect=true">
					<view class="d-flex align-items-center">
						<text class="font-size-32">{{selectCoin.text}}</text>
					</view>
					<view class="">
			...
			<view class="d-flex font-size-34 mt-28 withdraw_type">
				...
				<!-- <text class="item" :class="type == 3 ? 'active' : ''" @click="type=3">{{i18n.withdrawToCardOther}}</text> -->
			</view>
			<!-- 收款地址 -->
			...
			<template v-if="type == 3">
				<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 ">
					...
					<view class="input-group">
						<input type="text" v-if="card_other && card_other.data[0]" :value="card_other.data[0]" disabled class="input" >
						<button v-else class="warning-button py-0 font-size-22 w-50 mx-auto" @click="$utils.jump('/pages/setting/bank_other')">{{i18n.addCard}}</button>
					</view>
				</view>
			</template>
			...
			<text class="d-block text-right" style="height: 70rpx;line-height: 70rpx;">
				<text class="opacity-30">{{i18n.leastNumber}}</text> <text class="ml-26">{{min_number}} {{selectCoin.text}}</text>
			</text>
			<!-- 到账数量 -->
			<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 ">
				<view class="d-flex-between-center">
			...
			<!-- 手续费 -->
			<text class="d-block text-right" style="height: 70rpx;line-height: 70rpx;">
				<text class="opacity-30">{{i18n.handlingFee}}</text> <text class="ml-26">{{rate}} {{selectCoin.text}}</text>
			</text>
			...
			<u-popup v-model="showSelectAddress" mode="bottom">
				<view class="p-30">
					...
					<default-page v-else>
						<button class="w-50 mx-auto warning-button py-0 border-radius-20 text-white" @click="$utils.jump('/pages/setting/addWallet')">Add wallet address</button>
					</default-page>
				</view>
			</u-popup>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			withdraw(){
				const {i18n,selectAddress,number,rate,selectCoin,type,card,card_international,card_other} = this
				...
				if(!card_other.bank_account && type == 3){
					//没有银行卡
					return this.$utils.showToast(i18n.plsAddCard)
				}
				...
				this.$u.api.wallet.withdraw({address,currency,number,rate,type}).then(res=>{
			...
			getCardOther(){
				this.$u.api.setting.getCardOther().then(res=>{
					this.card_other = res.message
				})
			}
		},
		computed: {
