<template>
	<view class="pb-50">
		<u-navbar :title="assetsType.type"></u-navbar>
		<view class="deposit m-30">
			...
			<view class="mt-30">
				<view class="d-flex align-items-baseline">
					<text class="font-size-40 mr-20">{{balance.balance}}</text>
					<text class="font-size-22">USDT</text>
				</view>
			</view>
			<view class="d-block font-size-22 opacity-50 mt-8"> ≈ {{(balance.balance * $store.state.fiat.rate).toFixed(4)}} {{$store.state.fiat.currency_code}} </view>
	...
	export default {
		...
		methods: {
			getDetail() {
				...
				this.$u.api.wallet.getDetail(currency, type,this.isreal).then(res => {
					...
					if (this.type_id == 1) {
						...
						if(this.isreal==1){
							balance.balance = Number(res.message.moni_balance);
							balance.lock_balance = Number(res.message.lock_moni_balance);
						}
					}
			...
			getLegalLog() {
				...
				this.$u.api.wallet.getLegalLog(currency, type, page,this.isreal).then(res => {
