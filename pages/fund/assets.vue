		...
		<view class="m-30">
			<view class="deposit ">
				<view class="d-flex align-items-center">
					...
					<!-- <text :color="$baseColor" class="mr-20 isreal" @click="$utils.setReal(`/pages/fund/assets`)" v-if="isreal==0" >{{$t("real")}}</text>
					<text :color="$baseColor" class="mr-20 isreal" @click="$utils.setReal(`/pages/fund/assets`)" v-if="isreal==1">{{$t("demo")}}</text> -->
				</view>
				<view class="mt-30">
		...
		methods: {
			...
			getWalletList() {
				this.$u.api.wallet.getWalletList().then(res => {
					...
					this.originalList.moni_wallet.balance.forEach(item => {
						item.balance = Number(item.moni_balance)
						item.lock_balance = Number(item.lock_moni_balance)
						item.usdt_balance = parseFloat((Number(item.moni_balance) * Number(item
							.usdt_price)).toFixed(4))
					});
			...
			setShowList() {
				switch (this.current) {
					...
					case 2:
						...
						if(this.isreal==1){
							this.showList = this.originalList.moni_wallet.balance;
							}
