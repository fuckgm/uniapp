<template>
	<view class="box-shadow border-radius-20 position-relative pb-50 mt-70 animated w-100" :class="showData.show ? 'fadeInUp' : 'fadeOutLeft'"  @click="$utils.jump('/pages/nft/artworkDetail?id='+showData.id)">
		...
		<!-- 已开始 -->
		<view class="place-a-bid aa" v-if="showData.buy_status == 1" style="z-index: 99" >
	...
	export default {
		...
		computed:{
			...
			showData(){
				...
				// 0：已结束，1：已开始，2：未开始
				if (data.start_time) {
					data.start_time = Date.parse(data.start_time.replace(/-/g, '/'))
				}
				if (data.end_time) {
					data.end_time = Date.parse(data.end_time.replace(/-/g, '/')) 
				}
